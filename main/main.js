...
function SearchCol(input, col_index) {
    var query = $(input).val()
    $("#tabladatos").DataTable().column(col_index).search(query).draw()
}
...
jQuery(document).ready(function() {
    ...
    tablaUsuarios = $('#tabladatos').DataTable({
        //Buscar
    ...
    $('#tabladatos .search-input th').each(function(i) {
        var title = $(this).text();
        var events = 'onchange="SearchCol(this,' + i + ')"'
        events += ' onkeyup="SearchCol(this,' + i + ')"'
        $(this).html('<input type="text" ' + events + ' placeholder="Buscar en.. ' + title + '" />');
    });
...
*/
    ...
    /*$('#tabladatos thead2 tr').clone(true).appendTo('#tabladatos thead2');

    $('#tabladatos thead2 tr:eq(1) th').each(function(i) {
